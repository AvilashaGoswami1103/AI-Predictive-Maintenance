...
def generate_comparison_plots(df_real, df_predicted, output_dir):
    """
    Generates comparison plots between actual and predicted risk/anomaly scores.
    """
    ...
    if 'predicted_forecast' in df_predicted.columns:
    ...
    for status, color in risk_colors.items():
        ...
        if not subset_pred.empty and 'predicted_forecast' in subset_pred.columns:
